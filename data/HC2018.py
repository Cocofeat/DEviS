...
class HC(Dataset):
    def __init__(self, OOD_Condition='normal', Level=0, dataset_folder='/HC/',
    ...
    def __getitem__(self, item: int):
        if self.train_type in ['train', 'val']:
            ...
            if self.OOD_Condition == 'B':
                imgitk = np.load(self.condition_folder[item])
                image = imgitk.astype(np.float32)
            else:
                imgitk = sitk.ReadImage(self.folder[item])
                image = sitk.GetArrayFromImage(imgitk)
                image = image.astype(np.float32)
        ...
        labelitk = np.load(self.mask[item])
        ...
        if self.train_type in ['train', 'val']:
            ...
            if self.OOD_Condition == 'B':
                condition_imgitk = np.load(self.condition_folder[item])
                # condition_image = sitk.GetArrayFromImage(condition_imgitk)
                condition_image = condition_imgitk.astype(np.float32)
                sample = {'image': image, 'condition_image': condition_image, 'label': label}
            else:
                condition_image_itk = sitk.ReadImage(self.condition_folder[item])
                condition_image = sitk.GetArrayFromImage(condition_image_itk)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='TOAR', type=str, help="BraTS/ISIC/COVID/CHAOS/LiTS/TOAR")  #
