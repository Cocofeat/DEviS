...
class InitConv3D(nn.Module):
    def __init__(self, in_channels=4, out_channels=16, dropout=0.2):
        super(InitConv3D, self).__init__()
...
class EnBlock3D(nn.Module):
    def __init__(self, in_channels, norm='gn'):
        super(EnBlock3D, self).__init__()
...
class EnDown3D(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(EnDown3D, self).__init__()
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        model = Unet(in_channels=3, base_channels=32, num_classes=2)
        # model = Unet3D(in_channels=3, base_channels=32, num_classes=2)
        model.cuda()
        output = model(x)
