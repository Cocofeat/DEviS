...
from models.lib.PositionalEncoding import FixedPositionalEncoding,LearnedPositionalEncoding,LearnedPositionalEncoding3D
from models.lib.Unet_skipconnection import Unet,Unet3D
...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        if positional_encoding_type == "learned":
            self.position_encoding = LearnedPositionalEncoding3D(
        ...
        self.Unet = Unet3D(in_channels=num_channels, base_channels=16, num_classes=4)
...
class BTS(TransformerBTS):
    def __init__(
        ...
        self.Enblock8_1 = EnBlock13D(in_channels=self.embedding_dim)
        self.Enblock8_2 = EnBlock23D(in_channels=self.embedding_dim // 4)

        self.DeUp4 = DeUp_Cat3D(in_channels=self.embedding_dim//4, out_channels=self.embedding_dim//8)
        self.DeBlock4 = DeBlock3D(in_channels=self.embedding_dim//8)

        self.DeUp3 = DeUp_Cat3D(in_channels=self.embedding_dim//8, out_channels=self.embedding_dim//16)
        self.DeBlock3 = DeBlock3D(in_channels=self.embedding_dim//16)

        self.DeUp2 = DeUp_Cat3D(in_channels=self.embedding_dim//16, out_channels=self.embedding_dim//32)
        self.DeBlock2 = DeBlock3D(in_channels=self.embedding_dim//32)
...
def TransBTS(modal, _conv_repr=True, _pe_type="learned"):
    img_dim = 128
    num_classes = 4
    if modal == 'both':
        num_channels = 2 # 4 2 1
    else:
        num_channels = 1
    patch_dim = 8
    aux_layers = [1, 2, 3, 4]
...
class TransU2D(nn.Module):
    def __init__(
        ...
        super(TransU2D, self).__init__()

        assert embedding_dim % num_heads == 0
        assert img_dim1 % patch_dim == 0
        # assert img_dim2 % patch_dim == 0
        ...
        self.num_patches = int((img_dim1 // patch_dim) ** 3)
        self.seq_length = self.num_patches
        self.flatten_dim = 128 * num_channels
        ...
        if positional_encoding_type == "learned":
            self.position_encoding = LearnedPositionalEncoding(
                self.seq_length, self.embedding_dim, self.img_dim1
            )
        elif positional_encoding_type == "fixed":
            self.position_encoding = FixedPositionalEncoding(
                self.embedding_dim,
        ...
        self.Unet = Unet(in_channels=num_channels, base_channels=32, num_classes=2)
    ...
    def _reshape_output(self, x):
        x = x.view(
            x.size(0),
            int(self.img_dim1 / self.patch_dim),
            int(round(self.img_dim2 / self.patch_dim)),
            self.embedding_dim,
        )
...
class ISIC_TransU(TransU2D):
    def __init__(
        ...
        super(ISIC_TransU, self).__init__(
            img_dim1=img_dim1,
            img_dim2=img_dim2,
            patch_dim=patch_dim,
            num_channels=num_channels,
        ...
        self.DeUp4 = DeUp_OffsetCat(in_channels=self.embedding_dim//2, out_channels=self.embedding_dim//4)
        ...
        self.endconv = nn.Conv2d(self.embedding_dim // 16, num_classes, kernel_size=1)
...
class PTransU2D(nn.Module):
    def __init__(
        ...
        self.bn = nn.BatchNorm2d(256)
        self.relu = nn.ReLU(inplace=True)
        self.Prompt_Tokens = nn.Parameter(torch.zeros(num_layers, Prompt_Token_num, embedding_dim))
    ...
    def encode(self, x):
        ...
        for i in range(self.Prompt_Tokens.shape[0]):
            ...
            for name in range(self.num_heads):
                name = str(name)
                intmd_x[name] = intmd_x[name][:, :num_tokens - Prompt_Token_num]
...
class ISIC_PTransU(PTransU2D):
    def __init__(
        ...
        super(ISIC_PTransU, self).__init__(
            img_dim1=img_dim1,
            img_dim2=img_dim2,
            patch_dim=patch_dim,
            num_channels=num_channels,
...
class EnBlock2(nn.Module):
    ...
    def forward(self, x):
        ...
        return x1
class DeUp_OffsetCat(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(DeUp_OffsetCat, self).__init__()
    ...
    def forward(self, x, prev):
        ...
        y = self.conv2(x1)
        # y = y + prev
        y_offset = self.offset(y,prev)
        y = torch.cat((prev, y_offset), dim=1)
        y = self.conv3(y)
        return y
    ...
    def offset(self, inputs, down_outputs):
        ...
        out = inputs[:,:,:,:inputs.size()[3]-offset]
        return out
...
def Transformer_U(dataset, _conv_repr=True, _pe_type="learned"):
...
def PTransformer_U(dataset, _conv_repr=True, _pe_type="learned"):
