...
class Block(nn.Module):
    ...
    def load_from(self, weights, n_block):
        ...
        with torch.no_grad():
            kk = pjoin(ROOT, ATTENTION_Q, "kernel")
            print(kk)
...
class PTransformer(nn.Module):
    ...
    def forward(self, input_ids,Prompt_Tokensv):
        x, features = self.embeddings(input_ids)
        # add prompts
        Prompt_Token_num = Prompt_Tokensv.shape[1]
        attn_weights = []
        for i in range(Prompt_Tokensv.shape[0]):
            # concatenate Prompt_Tokens
            Prompt_Tokens = Prompt_Tokensv[i].unsqueeze(0)
            x = torch.cat((x, Prompt_Tokens.expand(x.shape[0], -1, -1)), dim=1)
            x, attn_weights = self.encoder(x)  # (B, n_patch, hidden)
...
class VPT_ViT(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843,Prompt_Token_num=1, VPT_type="Shallow", zero_head=False, vis=False):
        super(VPT_ViT, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier
        self.transformer = PTransformer(config, img_size, vis)
    ...
    def forward(self, x):
        if x.size()[1] == 1:
            x = x.repeat(1,3,1,1)
        x, attn_weights, features = self.transformer(x,self.Prompt_Tokens)  # (B, n_patch, hidden)
